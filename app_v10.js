...
function setAlarm() {
  ...
  isAlarmActive = true;
  snoozeCount = 0;
  localStorage.setItem("smart_alarm_total_reaction", 0);
...
function triggerAlarm() {
  ...
  const prevReaction = Number(localStorage.getItem("smart_alarm_total_reaction") || 0);
  const now = new Date();
  const reactionSec = (now - (alarmStartTime || now)) / 1000;
  const totalReaction = prevReaction + reactionSec;
  localStorage.setItem("smart_alarm_total_reaction", totalReaction);

  // 상태 판정
  let profile = "SOFT";
  if (totalReaction > 60 || snoozeCount >= 3) profile = "HARD";
  else if (totalReaction > 30 || snoozeCount >= 1) profile = "MEDIUM";
  ...
  console.log(`🔊 ${profile} 모드 (볼륨=${volume}, 주기=${beepInterval}ms)`);
...
function snoozeAlarm() {
  ...
  const reactionTime = (new Date() - (alarmStartTime || new Date())) / 1000;
  const prevReaction = Number(localStorage.getItem("smart_alarm_total_reaction") || 0);
  localStorage.setItem("smart_alarm_total_reaction", prevReaction + reactionTime);

  saveAlarmRecord("snooze", reactionTime, snoozeCount);
